...
const Home: NextPage = () => {
  ...
  const handleLocalUITargetChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if(event.target.value == stateFriend1Target || event.target.value == stateFriend2Target)
      setValidationTarget(true)
    else
      setValidationTarget(false)
  ...
  const handleLocalUICalldataChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if(event.target.value == stateFriend1Target || event.target.value == stateFriend2Target)
      setValidationCalldata(true)
    else
      setValidationCalldata(false)
  }
  ...
  const [validationTarget, setValidationTarget] = useState(false)
  const [validationCalldata, setValidationCalldata] = useState(false)
            ...
            <div>
              ...
              <div className='grid grid-cols-3 gap-4'>
                ...
                <label>Target Address: 
                  <input className={validationTarget ? styles.inptright : styles.inptwrong} type="text" value={stateMeTarget} onChange={handleLocalUITargetChange}></input>
                </label>
                <label>Desired Calldata: 
                  <input className={validationCalldata ? styles.inptright : styles.inptwrong} type="text" value={stateMeCalldata} onChange={handleLocalUICalldataChange}></input>
                </label>
